...
class App:
    ...
    COLORS: dict[str|int, tuple[int, int, int]] = {"bg": (205, 193, 181), "lines": (191, 175, 159), "dark_fg": (121, 111, 101), "light_fg": (247, 247, 237), 2: (238, 228, 218), 4: (236, 224, 202), 8: (242, 176, 121), 16: (245, 149, 101), 32: (245, 124, 95), 64: (245, 97, 59), 128: (237, 206, 113), 256: (237, 204, 99), 512: (236, 197, 60), 1024: (255, 190, 40), 2048: (255, 185, 0)}
    MOVE_DIRS: dict[str, tuple[int, int]] = {"UP": (-1, 0), "RIGHT": (0, 1), "DOWN": (1, 0), "LEFT": (0, -1)}
    ...
    def move(self, dir: str) -> None:
        match dir:
            case "UP":
                moved: bool = any([self.move_num(i, j, dir) for i in range(1, App.ROWS) for j in range(App.COLS)])
            case "DOWN":
                moved: bool = any([self.move_num(i, j, dir) for i in range(App.ROWS - 2, -1, -1) for j in range(App.COLS)])
            case "LEFT":
                moved: bool = any([self.move_num(i, j, dir) for j in range(1, App.COLS) for i in range(App.ROWS)])
            case "RIGHT":
                moved: bool = any([self.move_num(i, j, dir) for j in range(App.COLS - 2, -1, -1) for i in range(App.ROWS)])
        
        if self.check_empty():
            if moved: self.place_random()
        else:
            self.reset()
